...
show1() {
  ...
  int tt;
  enum opt {
    current = 1, all, search, sort, quit
  };
  do {
    tt = 0;
    if (!(strcmp(ss, "comp") == 0)) {
    ...
    if (stricmp(s1, "all") == 0)
      ch = all;
    if (stricmp(s1, "search") == 0)
      ch = search;
    if (stricmp(s1, "sorted") == 0)
      ch = sort;
    ...
    case search:
      ...
      conti: {
        ...
        for (i = 0; i < co; i++) {
          ...
          if (stricmp(s1, con[i].cn) == 0) {
            ...
            citi: {
              ...
              for (j = 0; j < ci; j++) {
                if (stricmp(s1, con[i].ct[j].city) == 0) {
                  ...
                  p("\nCOUNTRY\t\tCITY\tTEMPERATURE\tHUMID\tWIND\tPOLLUTE\tPRECIPITATION\n");
                  p("%s\n\t\t%s\t%7dC\t%3d%%\t%dkm/hr\t%3dPPM\t%7d%%\n\n", con[i].cn, con[i].ct[j].city, con[i].ct[j].wt.temp, con[i].ct[j].wt.humidity, con[i].ct[j].wt.wind, con[i].ct[j].wt.pollution, con[i].ct[j].wt.precipitation);
                  break;
                }
              }
            }
            if (j == ci) {
              cl;
              p("\a\n\n\n\t\tENTERED CITY IS NOT PRESENT IN THE LIST");
              sleep(1);
              cl;
              p("\t\t\t\tLIST\n\t\t\t\t----\n");
              for (j = 0; j < ci; j++) {
      ...
      if (i == co) {
        cl;
        p("\a\n\n\n\t\tENTERED COUNTRY IS NOT PRESENT IN THE LIST");
        sleep(1);
        cl;
        p("\t\t\t\tLIST\n\t\t\t\t----\n");
        for (j = 0; j < co; j++) {
    ...
    case sort:
      ...
      contin: {
        ...
        s("%s", s1);
        for (i = 0; i < co; i++) {
          if (strcmp(strupr(s1), con[i].cn) == 0) {
            du = con[i];
            int ii, jj, x[50];
            do {
