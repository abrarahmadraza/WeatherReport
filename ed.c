	  ...
	  f=fopen("weather.txt","w");fp=fopen("number.txt","w");
		...
		{  cl; p("\nINPUT COUNTRY %d:",i+1);
			...
			{  
				...
				ran();
				con[i].ct[j].wt.temp=dk.wt.temp;
				p("%d",con[i].ct[j].wt.temp);
				con[i].ct[j].wt.humidity=dk.wt.precipitation;
				
				con[i].ct[j].wt.wind=dk.wt.wind;
				
				con[i].ct[j].wt.pollution=dk.wt.pollution;
			
				con[i].ct[j].wt.precipitation=dk.wt.humidity;
			}fwrite(&con[i],sizeof(con[i]),1,f);
		}fclose(f);ch=0;
	  }
